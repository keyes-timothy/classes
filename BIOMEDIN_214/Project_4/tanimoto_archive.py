    ...
    def calc_all_tanimotos(self): 
        ...
        for i in range(len(self.drug_data)): #for each row in tanimoto matrix
            for j in range(len(self.drug_data)): #for each column in tanimoto matrix
                ...
                tanimoto_score = self.calc_tanimoto(drug_a, drug_b)
                self.tanimoto_matrix[i,j] = tanimoto_score
                
                #find whether the drugs have a shared target
                share_targets = self.has_common_target(drug_a, drug_b)
                self.target_matrix[i,j] = share_targets
...
def main():
    ...
    #parser.add_argument('', type = int, help = '', default = 1)
    #parser.add_argument('', type = int, help = '', default = 3)
    #parser.add_argument('', type = float, help = '', default = 0.999)
