...
class Protein(object):
    def __init__(self, sequence=None, pose=None):
        ...
        if sequence and pose:
            Exception('must give only sequence or pose, not both')
        if sequence:
    ...
    def init_extended(self):
		...
		Returns:
			None
        Side-effects: 
            Set torsions for each position in self (Protein object) from 1 to self.length to -150 for psi and 150 for phi. 
		"""
        for i in range(1, self.length+1):
            self.set_torsion(pos = i, phi = -150, psi = 150)
            
    def get_torsion(self, pos):
        """
        Get torsion angles at a defined position. You don't need to worry about omega angles
        ...
        """
        phi = self.pose.phi(pos)
        psi = self.pose.psi(pos)
